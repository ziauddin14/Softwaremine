...
import { Check, ArrowRight, Calendar, Users, Clock, LayoutDashboard, LineChart } from "lucide-react"
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        ...
        >
          ...
          <p className="text-muted-foreground max-w-3xl mx-auto text-lg">
            Primecare is a modern healthcare platform designed to help clinics manage appointments, patient records, doctor scheduling, and operational workflows in one place.
          </p>
        </motion.div>
            ...
            <div className="relative rounded-2xl border border-border/50 bg-card/80 backdrop-blur-xl p-1 shadow-2xl shadow-primary/10">
              ...
              <div className="relative rounded-xl bg-background/80 p-6">
                ...
                <div className="space-y-2">
                  <p className="text-sm font-medium text-muted-foreground mb-3">Upcoming Appointments</p>
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-secondary/20 border border-border/20">
                      <div className="flex items-center gap-3">
                        <span className="text-xs text-primary font-medium">{apt.time}</span>
                        <span className="text-sm text-foreground">{apt.patient}</span>
                      </div>
                      <span className="text-xs px-2 py-1 rounded-full bg-primary/10 text-primary">{apt.type}</span>
                    </div>
                  ))}
                </div>
              </div>
